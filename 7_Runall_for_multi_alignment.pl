...
main:{
	...
	my @pep_files=glob "$ARGV[0]/*pro.fasta";
...
sub multiply_alignment{
    my ($pep_file)=@_;  
    my $pwdname=$1, if($pep_file=~/^(.*?)\_pro.fasta/);
    `sed -i 's/-//g'  $pep_file`;
    `sed -i 's/-//g'  $pwdname\_dna.fasta`;
    system("einsi --thread 3  --quiet  $pep_file  > $pwdname\_pep_mafft.fasta");
    system(" /home/Ddong/leiming/bin/pal2nal.pl   $pwdname\_pep_mafft.fasta   $pwdname\_dna.fasta  -output  fasta  >  $pwdname\_cds_mafft.fasta");
}
